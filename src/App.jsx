...
export default function App() {
  ...
  function rotateDesk(id) {
    setDesks((d) =>
      d.map((desk) => (desk.id === id ? { ...desk, rotate: (desk.rotate + 90) % 360 } : desk))
    );
  }
  ...
  const seatsList = desks
    ...
    .flatMap((desk) => Array.from({ length: desk.seats }, (_, i) => `${desk.id}:${i}`));
  ...
  function saveSnapshot() {
    const data = { desks, boards, assignments, namesText, gridSize, snapToGrid };
